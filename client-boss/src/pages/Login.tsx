...
const Login: React.FC = () => {
  ...
  const handleRegister = async (values: any) => {
    setLoading(true);
    try {
      await register({ ...values, role: 'BOSS' });
  ...
  const loginForm = (
    <Form
      ...
      autoComplete="off"
      size="large"
      initialValues={{ username: 'boss1', password: '123456' }}
    >
      <Form.Item
        name="username"
    ...
    <div style={{
      ...
      <Card style={{ width: 400, boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: '#999' }}>老板端 - 找到你的专属大神</p>
        </div>
        <Tabs
          activeKey={activeTab}
        ...
        <div style={{ marginTop: 16, textAlign: 'center' }}>
          <Button type="link" onClick={() => window.open('http://localhost:3002', '_blank')}>
            前往大神端 →
          </Button>
