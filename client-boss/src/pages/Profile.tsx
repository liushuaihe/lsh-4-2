...
const Profile: React.FC = () => {
  ...
  const handleSave = async (values: any) => {
    try {
      const data = await request.put<User>('/auth/profile', values);
      setUser(data);
    ...
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      >
        <div style={{ padding: 16 }}>
          <Text>
            确定要切换到 <Text strong style={{ color: '#52c41a' }}>大神角色</Text> 吗？
          </Text>
          <div style={{ marginTop: 12, padding: 12, background: '#f5f7fa', borderRadius: 8 }}>
            ...
            <ul style={{ margin: '8px 0 0 20px', padding: 0 }}>
              <li style={{ fontSize: 12, color: '#666' }}>申请技能与语音认证</li>
              <li style={{ fontSize: 12, color: '#666' }}>设置接单状态</li>
              <li style={{ fontSize: 12, color: '#666' }}>管理订单和收益</li>
            </ul>
          </div>
        </div>
      </Modal>
